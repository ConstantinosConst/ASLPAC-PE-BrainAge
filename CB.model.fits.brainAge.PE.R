...
#   variable <- females_hc
#   
model.fits.brainAge.PE=function(model, brain_age_output){
  variable=model
  
  output <- read.csv(brain_age_output, header=T)
  variable$predAge <- output$x
  # names(variable)[names(variable) == "age"] <- "Age"
  variable$devAge <- (variable$predAge-variable$Age)
  variable$AE <- abs(variable$devAge)
  ##Create agebins.
  agebreaks <- c(19,20,21,22)
  agelabels <- c("19-19.99","20-20.99","21-21.99")
  ...
  variable$PE[variable$PE==0] <- "No_PE"
  variable$PE[variable$PE==1] <- "PE"
  
  for (i in c("No_PE","PE")){
    
    # Calculate model metrics
    
      variable.PE=variable[variable$PE==i,]
      cat("For model >",brain_age_output,", ",i,"< your MAE is: ",mae(variable.PE$Age, variable.PE$predAge), "\n")
      cat("For model >",brain_age_output,", ",i,"< your MSE is: ",mse(variable.PE$Age, variable.PE$predAge), "\n")
      cat("For model >",brain_age_output,", ",i,"< your age~predAge cor is: ",cor(variable.PE$Age, variable.PE$predAge), "\n")
      cat("For model >",brain_age_output,", ",i,"< your R2 is: ",caret::R2(variable.PE$Age, variable.PE$predAge), "\n")
      brain_PAD <- (variable.PE$predAge-variable.PE$Age)
    ...
    ##Make a plot of MAE distribution per PE status
      variable$PE=factor(variable$PE)
      p00 <- ggplot(variable, aes(y=AE, x=PE)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") + 
    ...
    ##Make a plot of MAE distribution per age bin and PE status
    variable$agegroups=factor(variable$agegroups)
    p02 <- ggplot(variable, aes(y=AE, x=agegroups, fill=PE)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") +
    ...
    # decrease font size for x-axis
    p02 <- p02 + theme(axis.text.x = element_text(size = 5)) 

    ##Make a plot of devAge distribution per PE status
       variable$PE=factor(variable$PE)
       p03 <- ggplot(variable, aes(y=devAge, x=PE)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") +
         geom_jitter(color="black", size=0.4, alpha=0.9)

       p03 <- p03 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                       panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))

    ##Make a plot of devAge distribution per age bin and PE
    variable$agegroups=factor(variable$agegroups)
    p04 <- ggplot(variable, aes(y=devAge, x=agegroups, fill=PE)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") +
      geom_jitter(color="black", size=0.2, alpha=0.9)
    ...
    # decrease font size for x-axis
    p04 <- p04 + theme(axis.text.x = element_text(size = 5))
    
    pdf(paste0(deparse(substitute(model)),"_PE_CB_BA_descriptives.pdf"))
    ...
    # How to evaluate models: observed vs. predicted or predicted vs. observed?. Ecological Modelling, 216(3-4), 316-322.
       ...
       variable$PE <- factor(variable$PE)
       p1 <- ggplot(variable, aes(x=Age, y=predAge, color=PE, group=PE))+ 
         geom_point(alpha=0.5) +
         geom_abline(a=0, b=1, color='black', linetype='dashed') + 
         scale_x_continuous(limits=(c(19,22)),breaks=c(19,20,21,22))+
         scale_y_continuous(limits=(c(0, 60)), breaks=c(0,10,20,30,40,50,60))+
         scale_colour_hue(l=50)+ geom_smooth(aes(fill=PE), method=lm, size=0.5, se=TRUE, fullrange=TRUE)+
